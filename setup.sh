...
if [ "$OSTYPE" = "linux-gnu" ]; then
	...
	echo "macOS detected."
	if [ ! -f xterm-256color-italic.terminfo ]; then echo "Warning: terminfo file xterm-256color-italic.terminfo not found."; else echo 'Installing terminfo for italic tmux...'; tic xterm-256color-italic.terminfo; fi
...
function install_dotfiles() {
	...
	ln -sfn $CUR_DIR/nvim/lua ~/.config/nvim/lua
	ln -sfn $CUR_DIR/nvim/syntax ~/.config/nvim/syntax
